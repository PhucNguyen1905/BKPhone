			...
			<tbody>
				...
				for ($i = 0; $i < $countOrder; $i++) {
					...
					if ($data["allOrder"][$i]['status'] == 0) {
						echo '<a href="http://localhost/BKPhone/OrderAdmin/updateStatusOrder/' . $data["allOrder"][$i]['id'] . '/1"><button  class="btn btn-sm btn-success" style="margin-bottom: 10px;">Duyệt</button><a/>
			<a href="http://localhost/BKPhone/OrderAdmin/updateStatusOrder/' . $data["allOrder"][$i]['id'] . '/2"><button class="btn btn-sm btn-danger">Hủy bỏ</button><a/>';
